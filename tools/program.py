...
def train(config,
    ...
    for epoch in range(start_epoch, epoch_num + 1):
        ...
        x = logs.split(', ')
        for p in x:
          list_logs.append(p.split(': ')[1])
        list_log = list_logs[1:]
        list_log.insert(0, str(epoch))
        list_1.append(list_log)
    ...
    logger.info(best_str)
    # ----------------------------------------------#
    filename = '/content/drive/MyDrive/PaddleOCR/result_parameter.csv'
    file_exists = os.path.isfile(filename)
    with open(filename, 'a', encoding='utf-8') as csvfile:
      headers = ['epoch', 'acc', 'norm_edit_dis', 'loss']
    ...
    filename = '/content/drive/MyDrive/PaddleOCR/result_accuracy.csv'
    file_exists = os.path.isfile(filename)
    with open(filename, 'a', encoding='utf-8') as csvfile:
      headers = ['epoch', 'acc', 'norm_edit_dis']
      writer = csv.writer(csvfile)
      if not file_exists:
          writer.writerow(headers)  # file doesn't exist yet, write a header
      for line in list_2:
